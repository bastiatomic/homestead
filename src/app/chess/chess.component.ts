...
export class ChessComponent {
  ...
  selectPosition(index: number): void {
    if (this.firstMove == null) {
      ...
      let index1 = this.firstMove;
      let index2 = this.secondMove;
  ...
  fenToBoard() {
    ...
    for (const char of fen) {
      ...
      this.board[currentBoardIndex].icon =
        'assets/chess/' + Mapping[char] + '.png';
  ...
  getPositionColor(index: number): string {
    ...
    const col = index % 8;
    return (row + col) % 2 === 0
      ? 'assets/legacy_assets/empty_tile_green_64x.png'
      : 'assets/legacy_assets/empty_tile_yellow_64x.png';
  }
}
