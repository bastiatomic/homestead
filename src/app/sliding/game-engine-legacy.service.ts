...
import { Board_Legacy } from './boardmap';

export interface visitedMap {
  [key: string]: Board_Legacy;
}
...
export class GameEngineService {
  ...
  playMove(board: Board_Legacy, id: number, direction: number): Board_Legacy | null {
  ...
  bfs(board: Board_Legacy): Board_Legacy[] {
    ...
    while (
      ...
      //console.log("------------")
      const frontierBoard : Board_Legacy = queue.shift()!// grab first element
  ...
  copyGameState(board:Board_Legacy) : Board_Legacy{
    //50% faster than JSON/Stringify
    let grid = board.board.map((item: number) => item)
    return {
      board: grid,
      width: board.width,
      height: board.height,
  ...
  backtracking(visitedMap: visitedMap, victoryIndexBoard: Board_Legacy): Board_Legacy[] {
